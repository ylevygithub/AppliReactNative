...
let constructBundleQueryParamsAsync = exports.constructBundleQueryParamsAsync = (() => {
  var _ref10 = _asyncToGenerator(function* (projectRoot, opts) {
    ...
    let usesAssetPluginsQueryParam = supportsAssetPlugins && (_Versions || _load_Versions()).lteSdkVersion(exp, '32.0.0');
    if (usesAssetPluginsQueryParam) {
      // Use an absolute path here so that we can not worry about symlinks/relative requires
      let pluginModule = (_config || _load_config()).resolveModule('expo/tools/hashAssetFiles', projectRoot, exp);
