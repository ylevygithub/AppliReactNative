...
let bundleAsync = exports.bundleAsync = (() => {
  var _ref = _asyncToGenerator(function* (context, assets, dest, oldFormat = false) {
    if (!assets) {
      return;
    }
    ...
    for (const batch of batches) {
      yield Promise.all(batch.map((() => {
        var _ref2 = _asyncToGenerator(function* (asset) {
          ...
          yield (0, (_ExponentTools || _load_ExponentTools()).saveUrlToPathAsync)(urlResolver(hash),
  ...
  return function bundleAsync(_x, _x2, _x3) {
...
function createAssetsUrlResolver(context) {
  ...
  if (context) {
    ...
    const { hostname } = _url.default.parse(publishedUrl);
    const maybeExpoDomain = (_lodash || _load_lodash()).default.takeRight(hostname.split('.'), 2).join('.');
    if (!(_lodash || _load_lodash()).default.includes(EXPO_DOMAINS, maybeExpoDomain)) {
