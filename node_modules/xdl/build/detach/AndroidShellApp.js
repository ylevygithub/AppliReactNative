...
let copyInitialShellAppFilesAsync = exports.copyInitialShellAppFilesAsync = (() => {
  var _ref2 = _asyncToGenerator(function* (androidSrcPath, shellPath, isDetached, sdkVersion) {
    ...
    const copyToShellApp = (() => {
      ...
      return function copyToShellApp(_x6) {
    ...
    if (parseSdkMajorVersion(sdkVersion) >= 32) {
      try {
        yield (_fsExtra || _load_fsExtra()).default.remove(_path.default.join(shellPath, 'app/src/main/assets/kernel.android.bundle'));
      } catch (e) {
  ...
  return function copyInitialShellAppFilesAsync(_x2, _x3, _x4, _x5) {
    return _ref2.apply(this, arguments);
  };
})();

let runShellAppModificationsAsync = exports.runShellAppModificationsAsync = (() => {
  var _ref5 = _asyncToGenerator(function* (context, sdkVersion) {
    ...
    const isDetached = (_ExponentTools || _load_ExponentTools()).parseSdkMajorVersion(sdkVersion) >= 32 || isRunningInUserContext;
    ...
    if (isDetached) {
      ...
      if (isRunningInUserContext) {
        yield regexFileAsync(/\/\* UNCOMMENT WHEN DETACHING/g, '', appBuildGradle);
        yield regexFileAsync(/END UNCOMMENT WHEN DETACHING \*\//g, '', appBuildGradle);
        yield deleteLinesInFileAsync('WHEN_DETACHING_REMOVE_FROM_HERE', 'WHEN_DETACHING_REMOVE_TO_HERE', appBuildGradle);
      }
      ...
      if (yield (_fsExtra || _load_fsExtra()).default.pathExists(runShPath)) {
    ...
    // Icons
    (0, (_AndroidIcons || _load_AndroidIcons()).createAndWriteIconsToPathAsync)(context, _path.default.join(shellPath, 'app', 'src', 'main', 'res'), isRunningInUserContext);
    ...
    if (backgroundImages && backgroundImages.length > 0) {
      ...
      yield Promise.all(backgroundImages.map((() => {
        var _ref6 = _asyncToGenerator(function* (image) {
          if (isRunningInUserContext) {
            // local file so just copy it
            yield (_fsExtra || _load_fsExtra()).default.copy(_path.default.resolve(context.data.projectPath, image.url), image.path);
          } else {
    ...
    yield (_AssetBundle || _load_AssetBundle()).bundleAsync(context, manifest.bundledAssets, `${shellPath}/app/src/main/assets`);
  ...
  return function runShellAppModificationsAsync(_x8, _x9) {
    return _ref5.apply(this, arguments);
  };
})();

let buildShellAppAsync = (() => {
  var _ref7 = _asyncToGenerator(function* (context, sdkVersion) {
    let shellPath = shellPathForContext(context);
    ...
    if (context.build.android) {
      ...
      if ((_ExponentTools || _load_ExponentTools()).parseSdkMajorVersion(sdkVersion) >= 33) {
        gradleBuildCommand = 'assembleRelease';
      } else if ((_ExponentTools || _load_ExponentTools()).parseSdkMajorVersion(sdkVersion) >= 32) {
      ...
      if ((_ExponentTools || _load_ExponentTools()).parseSdkMajorVersion(sdkVersion) >= 32) {
        let apkPath;
        if ((_ExponentTools || _load_ExponentTools()).parseSdkMajorVersion(sdkVersion) >= 33) {
          apkPath = _path.default.join(shellPath, 'app', 'build', 'outputs', 'apk', 'release', 'app-release.apk');
        } else {
          apkPath = _path.default.join(shellPath, 'app', 'build', 'outputs', 'apk', 'prodKernel', 'release', 'app-prodKernel-release.apk');
        }
        yield (_fsExtra || _load_fsExtra()).default.copy(apkPath, 'shell.apk');
        // -c means "only verify"
        yield spawnAsync(`zipalign`, ['-c', '-v', '4', 'shell.apk'], {
          pipeToLogger: true,
      ...
      yield spawnAsyncThrowError(`./gradlew`, [gradleBuildCommand], {
        pipeToLogger: true,
        loggerFields: { buildPhase: 'running gradle' },
        cwd: shellPath
      });
      let apkPath;
      if ((_ExponentTools || _load_ExponentTools()).parseSdkMajorVersion(sdkVersion) >= 33) {
        apkPath = _path.default.join(shellPath, 'app', 'build', 'outputs', 'apk', 'debug', 'app-debug.apk');
      } else if ((_ExponentTools || _load_ExponentTools()).parseSdkMajorVersion(sdkVersion) >= 32) {
        apkPath = _path.default.join(shellPath, 'app', 'build', 'outputs', 'apk', 'devKernel', 'debug', 'app-devKernel-debug.apk');
      } else {
        apkPath = _path.default.join(shellPath, 'app', 'build', 'outputs', 'apk', 'devMinSdkDevKernel', 'debug', 'app-devMinSdk-devKernel-debug.apk');
      }
      yield (_fsExtra || _load_fsExtra()).default.copy(apkPath, `/tmp/shell-debug.apk`);
    }
  });

  return function buildShellAppAsync(_x11, _x12) {
...
let removeObsoleteSdks = (() => {
  ...
  return function removeObsoleteSdks(_x15, _x16) {
    return _ref9.apply(this, arguments);
  };
})();

let prepareEnabledModules = (() => {
  var _ref10 = _asyncToGenerator(function* (shellPath, modules) {
    ...
    yield (_fsExtra || _load_fsExtra()).default.remove(enabledModulesDir);
    if (!modules) {
      yield (_fsExtra || _load_fsExtra()).default.ensureSymlink(packagesDir, enabledModulesDir);
    } else {
      yield (_fsExtra || _load_fsExtra()).default.mkdirp(enabledModulesDir);
      yield Promise.all(modules.map(function (mod) {
        return (_fsExtra || _load_fsExtra()).default.ensureSymlink(_path.default.join(packagesDir, mod.dirname), _path.default.join(enabledModulesDir, mod.dirname));
      }));
    }
  });

  return function prepareEnabledModules(_x17, _x18) {
...
function backgroundImagesForApp(shellPath, manifest, isDetached, sdkVersion) {
  ...
  if ((_lodash || _load_lodash()).default.get(manifest, 'android.splash')) {
    const splash = (_lodash || _load_lodash()).default.get(manifest, 'android.splash');
    const results = (_lodash || _load_lodash()).default.reduce(imageKeys, function (acc, imageKey) {
...
exports.createAndroidShellAppAsync = (() => {
  var _ref4 = _asyncToGenerator(function* (args) {
    ...
    yield copyInitialShellAppFilesAsync(androidSrcPath, shellPath, false, sdkVersion);
    yield removeObsoleteSdks(shellPath, sdkVersion);
    yield runShellAppModificationsAsync(context, sdkVersion);
    yield prepareEnabledModules(shellPath, modules);

    if (!args.skipBuild) {
      yield buildShellAppAsync(context, sdkVersion);
    }
  });

  function createAndroidShellAppAsync(_x7) {
...
const removeInvalidSdkLinesWhenPreparingShell = (() => {
  ...
  return function removeInvalidSdkLinesWhenPreparingShell(_x13, _x14) {
