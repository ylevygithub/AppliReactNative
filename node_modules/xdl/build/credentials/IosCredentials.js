...
let getExistingDistCerts = exports.getExistingDistCerts = (() => {
  var _ref = _asyncToGenerator(function* (username, appleTeamId, options = {}) {
...
let getExistingPushKeys = exports.getExistingPushKeys = (() => {
  var _ref2 = _asyncToGenerator(function* (username, appleTeamId, options = {}) {
    const pushKeys = yield getExistingUserCredentials(username, appleTeamId, 'push-key');
    return pushKeys.map(function ({ usedByApps, userCredentialsId, apnsKeyId, apnsKeyP8 }) {
      let name = `Key ID: ${apnsKeyId}`;
      if (usedByApps) {
        name = `Used in apps: ${usedByApps.join(', ')} (${name})`;
      }
      return {
        value: _extends({}, options.provideFullPushKey ? { apnsKeyId, apnsKeyP8 } : { userCredentialsId: String(userCredentialsId) }),
        name,
        short: apnsKeyId
      };
    });
...
function formatDistCerts(distCerts, options) {
  return distCerts.map(({ usedByApps, userCredentialsId, certId, certP12, certPassword }) => {
    ...
    return {
      ...
      }, options.provideFullCertificate ? { certP12, certId, certPassword } : { userCredentialsId: String(userCredentialsId) }),
      name
    };
  });
}
