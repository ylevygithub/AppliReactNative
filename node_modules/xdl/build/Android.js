...
exports.checkSplashScreenImages = exports.stopAdbReverseAsync = exports.startAdbReverseAsync = exports.openProjectAsync = exports.upgradeExpoAsync = undefined;
...
let checkSplashScreenImages = exports.checkSplashScreenImages = (() => {
  var _ref19 = _asyncToGenerator(function* (projectDir) {
    const { exp } = yield (_ProjectUtils || _load_ProjectUtils()).readConfigJsonAsync(projectDir);
    const splashScreenMode = (_lodash || _load_lodash()).default.get('android.splash.resizeMode') || (_lodash || _load_lodash()).default.get(exp, 'splash.resizeMode');
    if (splashScreenMode === 'contain') {
      return;
    }

    const generalSplashImagePath = (_lodash || _load_lodash()).default.get(exp, 'splash.image');
    if (!generalSplashImagePath) {
      (_Logger || _load_Logger()).default.global.warn(`Couldn't read '${(_chalk || _load_chalk()).default.italic('splash.image')}' from ${(_chalk || _load_chalk()).default.italic('app.json')}. Provide asset that would serve as baseline splash image.`);
      return;
    }
    const generalSplashImage = yield (0, (_ImageUtils || _load_ImageUtils()).getImageDimensionsAsync)(projectDir, generalSplashImagePath);
    if (!generalSplashImage) {
      (_Logger || _load_Logger()).default.global.warn(`Couldn't read dimensions of provided splash image '${(_chalk || _load_chalk()).default.italic(generalSplashImagePath)}'. Does the file exist?`);
      return;
    }

    const androidSplash = (_lodash || _load_lodash()).default.get(exp, 'android.splash');
    const androidSplashImages = [];
    for (const _ref20 of splashScreenDPIConstraints) {
      const { dpi, sizeMultiplier } = _ref20;

      const imageRelativePath = (_lodash || _load_lodash()).default.get(androidSplash, dpi);
      if (imageRelativePath) {
        const splashImage = yield (0, (_ImageUtils || _load_ImageUtils()).getImageDimensionsAsync)(projectDir, imageRelativePath);
        if (!splashImage) {
          (_Logger || _load_Logger()).default.global.warn(`Couldn't read dimensions of provided splash image '${(_chalk || _load_chalk()).default.italic(splashImage)}'. Does the file exist?`);
          continue;
        }
    ...
    if (androidSplashImages.length === 0) {
      (_Logger || _load_Logger()).default.global.warn(`Splash resizeMode is set to 'cover', but you haven't provided any images for different DPIs.
Be aware that your splash image will be used as xxxhdpi asset and its ${(_chalk || _load_chalk()).default.bold('actual size will be different')} depending on device's DPI.
See https://docs.expo.io/versions/latest/guides/splash-screens/#differences-between-environments---android for more information`);
    ...
    if ((_lodash || _load_lodash()).default.some(androidSplashImages, function ({ sizeMatches }) {
      return !sizeMatches;
    })) {
      (_Logger || _load_Logger()).default.global.warn(`Splash resizeMode is set to 'cover' and you've provided different images for different DPIs,
but their sizes mismatch expected ones: [dpi: provided (expected)] ${androidSplashImages.map(function ({ dpi, width, height, expectedWidth, expectedHeight }) {
        return `${dpi}: ${width}x${height} (${expectedWidth}x${expectedHeight})`;
      }).join(', ')}
See https://docs.expo.io/versions/latest/guides/splash-screens/#differences-between-environments---android for more information`);
    }
  });

  return function checkSplashScreenImages(_x9) {
...
function _apkCacheDirectory() {
  ...
  (_fsExtra || _load_fsExtra()).default.mkdirpSync(dir);
  return dir;
}
